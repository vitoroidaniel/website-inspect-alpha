...
app.post('/api/driver/login', (req, res) => {
  ...
  if (!user || user.driver_pin !== String(pin)) return res.status(401).json({ error: 'Invalid PIN' });
...
// Inspection steps by type
app.get('/api/inspection-steps', requireAuth, (req, res) => {
  const type = req.query.type || 'pickup';
  const steps = db.prepare('SELECT * FROM inspection_steps WHERE active=1 AND inspection_type=? ORDER BY step_number').all(type);
  res.json(steps);
});
...
app.post('/api/inspections/start', requireDriver, (req, res) => {
  ...
  const type = req.body.inspection_type || 'pickup';
...
app.post('/api/admin/users', requireAdmin, (req, res) => {
  ...
  if (role === 'driver') {
    ...
    const fakeHash = bcrypt.hashSync(uuidv4(), 10);
...
app.get('/api/admin/steps', requireAgent, (req, res) => {
  res.json(db.prepare('SELECT * FROM inspection_steps ORDER BY inspection_type, step_number').all());
});
...
app.listen(PORT, () => console.log(`FleetInspect v3 running on port ${PORT}`));