...
app.post('/api/inspections/start', driver, (req,res) => {
  ...
  db.prepare(`INSERT INTO inspections (id,driver_id,driver_name,truck_model,truck_number,inspection_type,status,started_at) VALUES (?,?,?,?,?,?,'in_progress',NOW())`).run(id,u.id,u.name,u.truck_model||'N/A',u.truck_number||'',type);
  ...
  upload.single('photo')(req, res, async (err) => {
    ...
    try {
      ...
      await sharp(req.file.path)
        .jpeg({ quality: 70 }) // 70% quality - good compression
        .resize(1920, 1080, { fit: 'inside', withoutEnlargement: true }) // Max 1080p
      ...
      await db.prepare(`INSERT OR REPLACE INTO inspection_photos (inspection_id,step_number,step_label,file_path,latitude,longitude,taken_at) VALUES (?,?,?,?,?,?,NOW())`)
...
app.get('/api/agent/inspections/:id/download', agent, (req, res) => {
  ...
  db.prepare('SELECT * FROM inspection_photos WHERE inspection_id=? ORDER BY step_number').all(req.params.id).then(photos => {
...
app.get('/api/agent/stats', agent, async (req, res) => {
  ...
  const todayInspections = await db.prepare("SELECT COUNT(*) as c FROM inspections WHERE status='submitted' AND date(submitted_at)=date('now')").get();
  const totalPhotos = await db.prepare("SELECT COUNT(*) as c FROM inspection_photos").get();
  
  res.json({
    totalDrivers: totalDrivers?.c || 0,
    totalDispatchers: totalDispatchers?.c || 0,
    totalInspections: totalInspections?.c || 0,
    todayInspections: todayInspections?.c || 0,
...
app.post('/api/admin/users', admin, (req, res) => {
  ...
  db.prepare('INSERT INTO users (username,email,password_hash,full_name,role,truck_model,truck_number) VALUES (?,?,?,?,?,?,?)')
...
app.put('/api/admin/users/:id', admin, async (req, res) => {
  ...
  if (username && username !== user.username) {
    const taken = await db.prepare('SELECT id FROM users WHERE username=? AND id!=?').get(username, req.params.id);
    if (taken) return res.status(409).json({ error: 'Username taken' });
  }
  
  if (email !== undefined && email !== user.email) {
    const inUse = await db.prepare('SELECT id FROM users WHERE email=? AND id!=?').get(email, req.params.id);
    if (inUse) return res.status(409).json({ error: 'Email in use' });
  }
...
app.post('/api/admin/steps', admin, (req, res) => {
  ...
  const max = db.prepare('SELECT MAX(step_number) as m FROM inspection_steps WHERE inspection_type=?').get(type);
  const nextStep = (max?.m || 0) + 1;
  
  db.prepare('INSERT INTO inspection_steps (inspection_type,step_number,label,instruction) VALUES (?,?,?,?)').run(type, nextStep, label, instruction);
  res.json({ ok: true });
});
